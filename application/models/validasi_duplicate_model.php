...
class Validasi_Duplicate_Model extends BaseModel{
    public function __construct(){
        parent::__construct($this);
    }
    public function countRowD(){
    $query = $this->db->query('SELECT * FROM valadasi_duplicate');
    return $query->num_rows();
    }
    public function showID($id){
    $query = $this->db->query('SELECT * FROM validasi_duplicate where id_mbr ="'.$id.'"');
    return $query->result_array();
    }
    public function showD(){
    $query = $this->db->query('SELECT * FROM validasi_duplicate');
    return $query->result_array();
    }
    public function errorK(){
    $query = $this->db->query("SELECT * FROM validasi_duplicate where val_status != 'LAMA' and gabung = 'BELUM ADA KETERANGAN'");
    return $query->result_array();
    }
    public function errorS(){
    $query = $this->db->query("SELECT * FROM validasi_duplicate where val_status != 'LAMA' and surveyor_id = 1 or surveyor_id = 0 or surveyor_id = 73");
    return $query->result_array();
    }
    public function listS($number){
    $query = $this->db->query('SELECT * FROM validasi_duplicate where id_surveyor ='.$number);
    return $query->result_array();
    }
    ...
    function list($id= '')
    {
        ...
        $this->db->from('validasi_duplicate as val');
